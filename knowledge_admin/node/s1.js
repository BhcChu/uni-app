...
var options = {
    key: fs.readFileSync('/www/server/panel/vhost/cert/demo1.sdwanyue.com/privkey.pem'),
    cert: fs.readFileSync('/www/server/panel/vhost/cert/demo1.sdwanyue.com/fullchain.pem')
};
	...
	socket.on('broadcast',function(data){
		    ...
		    if(socket.token != undefined){
			    ...
			    switch(method){
                    ...
                    case 'LinkMic':{     //连麦(举手)  action  1学生申请 2老师同意3老师拒绝 4学生主动下麦 5老师让学生下麦  6老师让学生上麦  7学生同意上麦 8学生拒绝上麦 9学生取消上麦
                        if(action==3 ){
                            var touid=msg['touid'];
                            cancelLinkmic(touid,socket.stream);
                        }
                        
                        if(action==4 ){
                            var touid=msg['uid'];
                            quitLinkmic(touid,socket.stream);
                        }
                    ...
                    case 'setWhite' :{//白板操作  action  1老师邀请上台 2学生同意3学生拒绝4学生主动下台 5 老师让学生下台
                        if(action==1 || action==5){
                            if(socket.usertype > 0){
                                if(action==1){
                                    var touid=msg['touid'];
                                    setWrite(touid,socket.stream,1);
                                }
                                if(action==5){
                                    var touid=msg['touid'];
                                    setWrite(touid,socket.stream);
                                }
                                process_msg(io,socket.stream,data_str);
                            }
                            break;
                    ...
                    case 'exam' :{//测  action 0结束  1发布 3回答
                    ...
                    case 'practice' :{//练  action 0结束  1发布 2结束答题 3回答
                        if(action==0 || action==1){
                            if(socket.usertype == 1){
                                process_msg(io,socket.stream,data_str);
                            }
                    ...
                    case 'know' :{//懂  action 0结束  1发布 3回答
                        if(action==0 || action==1){
                            if(socket.usertype == 1){
                                process_msg(io,socket.stream,data_str);
                            }
...
/* 白板授权 */
function setWrite(uid,stream,type=0){
    if(type==1){
        clientRedis.hset('write_'+stream,uid,1);
    }else{
        clientRedis.hdel('write_'+stream,uid);
    }
}
